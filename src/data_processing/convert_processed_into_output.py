...
complete_output_clearing_df = pd.DataFrame()

input_df = pd.DataFrame()
input_clearing_df = pd.DataFrame()
complete_input_clearing_df = pd.DataFrame()
...
for sales_order_dir in last_processed_step_dir.iterdir():
    if sales_order_dir.is_dir():
        ...
        input_data = pd.read_csv(input_file, dtype={"terms_of_payment": str, "so_date": str, "req_deliv_date": str})
        ...
        if file_not_empty(clearing_invoices_file):
            ...
            if len(clearing_invoices.index) == len(invoices.index):
                ...
                complete_output_clearing_df = pd.concat([complete_output_clearing_df, clearing_invoices], ignore_index=True)
                complete_input_clearing_df = pd.concat([complete_input_clearing_df, input_data], ignore_index=True)
...
log(f"Total invoices w/ complete clearings: {len(complete_output_clearing_df.index)} ({len(complete_output_clearing_df.index) / len(output_df.index) * 100}%)")
